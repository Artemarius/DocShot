...
- **Phase 11:** Capture UX & auto-capture quality — freeze overlay + auto-capture hardening complete; aspect ratio slider + AF lock remaining; quad tracking deprioritized
...
- Phase 11 in progress: 11A + 11A+ complete. Freeze overlay fixed (analyzer guard + 75% scrim). Auto-capture hardened: 1.5s warmup suppression, stableThreshold 10→20, three-tier drift response (increment/halve/hard-reset), pre-smoothing jump detection (>10% deviation clears buffer), confidence formula rebalanced (removed area double-counting, edge density 20%→40%). Debug overlay added (stable N/20, conf, warmup, READY). Gradient-based quad tracking (11D) deprioritized — smoother hardening is sufficient for v1. Remaining: 11B (aspect ratio slider) and 11C (AF lock).
