...
- Phase 7 in progress (Groups A+B+C complete): per-detection confidence scoring (weighted: 60% quad score + 20% area ratio + 20% edge density), edge-density validation via QuadValidator (perimeter edge-pixel sampling), confidence suppression threshold at 0.35 to reduce false positives. Confidence wired through full pipeline: DocumentCorners → FrameAnalyzer → QuadSmoother (rolling average) → FrameDetectionResult → CaptureResult. Group B: low-confidence fallback (captures with confidence 0.35–0.65 route to CornerAdjustScreen for manual adjustment instead of Result), "Point at a document" hint on camera screen when no detection, auto-capture gated to high-confidence (>=0.65) only. Multi-candidate score-margin factor in QuadRanker (rankQuads returns candidateCount + scoreMargin; ambiguous multi-candidate scenes get up to 50% confidence penalty). 5 new unit tests for rankQuads. Group C: multi-strategy preprocessing (STANDARD, CLAHE_ENHANCED, SATURATION_CHANNEL, BILATERAL, HEAVY_MORPH) with scene analysis and 25ms time budget — handles low light, low contrast, colored backgrounds, patterned surfaces. CLAHE always included as fallback after STANDARD (scene analysis alone misses moderate-contrast scenes); uses 5x5 Gaussian (not 9x9) and fixed lower Canny thresholds (30/60) to preserve subtle edge gradients. Small document support (MIN_AREA_RATIO lowered to 2%, aspect-ratio scoring with Gaussian falloff against known formats, scoring weights 40% area + 40% angle + 20% aspect). Partial document detection (large contour or valid quad touching 2+ frame edges → "Move back to fit document" hint). Synthetic test infrastructure (SyntheticImageFactory + 11 instrumented regression tests, all passing). 9 new unit tests (4 contour analysis + 5 aspect ratio).
